...
class Pingsweep(NotifyBase):
	...
	def _run(self):
		...
		process  = subprocess.Popen("%s | grep -v latency | grep -v Start | grep -v done > %s.new" % (command,target), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
		out, err = process.communicate()

		self._completed = False

		if not err:
			process  = subprocess.Popen("cat %s.new" % (target), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
			out, err = process.communicate()
